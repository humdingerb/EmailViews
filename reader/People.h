...
class Person {
public:
	explicit				Person(const entry_ref& ref);
	virtual					~Person();
...
public:
	explicit				PersonList(QueryList& query);
							~PersonList() override;
	...
	virtual	void			EntryCreated(QueryList& source,
								const entry_ref& ref, ino_t node) override;
	virtual	void			EntryRemoved(QueryList& source,
								const node_ref& nodeRef) override;
...
public:
	explicit				GroupList(QueryList& query);
							~GroupList() override;
