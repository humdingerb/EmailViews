...
public:
	...
	virtual void FetchChoicesFor(const BString& pattern)
	{
		...
		PersonList& peopleList = gReaderSettings->People();
...
{
	if (gReaderSettings != NULL)
		gReaderSettings->PeopleQueryList().AddListener(this);
}
...
{
	if (gReaderSettings != NULL)
		gReaderSettings->PeopleQueryList().RemoveListener(this);
}
...
AddressPopUpMenu::_RebuildMenu()
{
	...
	GroupList& groupList = gReaderSettings->PeopleGroups();
	...
	fRefDropMenu(NULL)
{
	...
	fTextView->SetFlags(B_WILL_DRAW | B_NAVIGABLE | B_INPUT_METHOD_AWARE);
